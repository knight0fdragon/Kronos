...
//////////////////////////////////////////////////////////////////////////////
 ...
 u8 FASTCALL FlashCs0ReadByte(SH2_struct* context, UNUSED u8* memory, u32 addr)
{
	flashstate* state;
	u8* reg;
...
//////////////////////////////////////////////////////////////////////////////
 ...
 void FASTCALL FlashCs0WriteByte(SH2_struct* context, UNUSED u8* memory, u32 addr, u8 val)
{
	flashstate* state;
	u8* reg;
	u8* buf;
...
//////////////////////////////////////////////////////////////////////////////
 ...
 u8 FASTCALL AR4MCs0ReadByte(SH2_struct* context, UNUSED u8* memory, u32 addr)
{
	addr &= 0x1FFFFFF;
	switch (addr >> 20)
	{
...
//////////////////////////////////////////////////////////////////////////////
 ...
 void FASTCALL AR4MCs0WriteByte(SH2_struct* context, UNUSED u8* memory, u32 addr, u8 val)
{
	addr &= 0x1FFFFFF;
...
int CartReset()
{
	if (NetlinkArea) {
		...
		if (strlen(NetlinkArea->ipstring) > 0) {
			if ((ipstring = malloc(strlen(NetlinkArea->ipstring) + 1)) > 0)
				memcpy(ipstring, NetlinkArea->ipstring, strlen(NetlinkArea->ipstring) + 1);
		}
		if (strlen(NetlinkArea->portstring) > 0) {
			if (portstring = malloc(strlen(NetlinkArea->portstring) + 1))
				memcpy(portstring, NetlinkArea->portstring, strlen(NetlinkArea->portstring) + 1);
		}

		NetlinkDeInit();

		NetlinkInit(ipstring, portstring);
	}
	...
	if (USBArea) {
		...
		if (strlen(USBArea->ipstring) > 0) {
			if ((ipstring = malloc(strlen(USBArea->ipstring) + 1)) > 0)
				memcpy(ipstring, USBArea->ipstring, strlen(USBArea->ipstring) + 1);
		}
		if (strlen(USBArea->portstring) > 0) {
			if (portstring = malloc(strlen(USBArea->portstring) + 1))
				memcpy(portstring, USBArea->portstring, strlen(USBArea->portstring) + 1);
		}

		USBDeInit();

		USBInit(ipstring, portstring);
	}

}
int CartInit(const char* filename, int type, const char* modemip, const char* modemport)
{
	...
	case CART_USBDEV: // USB Dev Cartridge
	{
		...
		USBInit(modemip, modemport);
		// Setup Functions
		CartridgeArea->Cs0ReadByte = &USBReadByte;
		CartridgeArea->Cs0ReadWord = &AR4MCs0ReadWord;
		CartridgeArea->Cs0ReadLong = &AR4MCs0ReadLong;
		CartridgeArea->Cs0WriteByte = &USBWriteByte;
		CartridgeArea->Cs0WriteWord = &AR4MCs0WriteWord;
		CartridgeArea->Cs0WriteLong = &AR4MCs0WriteLong;
		break;
	}
...
int CartLoadState(const void* stream, UNUSED int version, int size)
{
	...
	if (newtype == CART_DRAM8MBIT || newtype == CART_DRAM32MBIT) {
		if (newtype != CartridgeArea->carttype) {
			CartDeInit();
			CartInit(NULL, newtype, NULL, NULL);
		}
