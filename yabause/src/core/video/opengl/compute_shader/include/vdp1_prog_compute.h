...
"}\n"
"bool onChange(ivec2 P, ivec2 P0, float slope, ivec2 a, int orientation, int cidx, int lidx) {\n"
...
"  }\n"
"}\n"
"bool onChangeOpposite(ivec2 P, ivec2 P0, float slope, ivec2 a, int orientation, int cidx, int lidx) {\n"
"  if ((a.x == a.y)^^(orientation==1)) {\n"
"   if (cidx>=lidx) return false;\n"
"   if (!isOnLine(P+ivec2(-a.x,0), P0, slope)) return false;\n"
"   if (!isOnLine(P+ivec2(0, a.y), P0, slope)) return false;\n"
...
"  }\n"
"}\n"
"bool onChangeAliased(ivec2 P, ivec2 P0, float slope, ivec2 a, int orientation, int cidx, int lidx) {\n"
"  if ((a.x == a.y)^^(orientation==1)) {\n"
"   if (cidx>lidx) {\n"
"    if (!isOnLine(P+ivec2(2*a.x,0), P0, slope)) return false;\n"
"    if (!isOnLine(P+ivec2(0,-a.y), P0, slope)) return false;\n"
"    return true;\n"
"   }\n"
"   if (!isOnLine(P+ivec2( a.x,0), P0, slope)) return false;\n"
...
"  float slope = 1.0;\n"
"  int lidx =  abs(Pn.y-P0.y);\n"
"  int cidx =  abs(Pn.x-P0.x);\n"
...
"  }\n"
"  if (!pixelOnLine) pixelOnLine = (onChange(Pn, P0, slope, a, int(orientation), cidx, lidx));\n"
"  if (!pixelOnLine) pixelOnLine = onChangeOpposite(Pn, P0, slope, a, int(orientation), cidx, lidx);\n"
